//
//  umeSothukokusaiViewController.swift
//  fmdb_start
//
...
class umeSothukokusaiViewController: UIViewController,UITextFieldDelegate,MMPopLabelDelegate,UIPickerViewDelegate,UIPickerViewDataSource{
    
    
    @IBAction func unwindActionToUmeOnKokusai(segue: UIStoryboardSegue) {
        isTapped = 1
        if segue.identifier == "lecBtnToUmeOnKokusai"{
            textFieldBorderEnable(TorF: true)
        }else if segue.identifier == "barBackBtnToUmeOnKokusai"{
            textFieldBorderEnable(TorF: true)
        }
    ...
    @IBOutlet weak var btnAorB: UIButton!
    
    @IBOutlet weak var btn4and5: UIButton!
    ...
    override func viewDidLoad() {
        ...
        umeKomaHeight = typeView.bounds.height / 20
        ...
        let doneItem = UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(umeSothukokusaiViewController.kannryoTapped))
        ...
        //専攻群のとこの文字の初期設定
        if (userDefault.string(forKey: "selectGunKokusai") != nil){
            typeThree[1].text = userDefault.string(forKey: "selectGunKokusai")! + "選択※"
        }else{
            typeThree[1].text = "3群A・B選択※"
        }
        
        
        //左のタイプラベルの大きさ
            typeOne.heightAnchor.constraint(equalTo: typeView.heightAnchor, multiplier: 1.0/20.0).isActive = true
        
        for i in 0..<typeTwo.count{
            typeTwo[i].heightAnchor.constraint(equalTo: typeView.heightAnchor, multiplier: 2.0/20.0).isActive = true
        }
        
        for i in 0..<typeThree.count{
            typeThree[i].heightAnchor.constraint(equalTo: typeView.heightAnchor, multiplier: 3.0/20.0).isActive = true
        }
        
        typeFive.heightAnchor.constraint(equalTo: typeView.heightAnchor, multiplier: 5.0/20.0).isActive = true
    ...
    @objc func clickEditButton(){
        ...
        for i in 0..<txtList.count{
            ...
            if (txtList[i].text?.isEmpty)!{
                ...
                //3群のA.Bの埋めのところで群が未選択の時は押したらカラカラを出す
                if (userDefault.integer(forKey: "selectGunKokusaiNum") == 0){
                    //専攻群のtextField
                    if i >= 27 && i <= 33{
                        txtList[i].addTarget(self, action: #selector(umeSothukokusaiViewController.AorBButton), for: .editingDidBegin)
                        continue
                    }
                    //群選択がされている時はカラカラ出す処理を削除しておく
                }else{
                    
                    txtList[i].removeTarget(self, action: #selector(umeSothukokusaiViewController.AorBButton), for: .editingDidBegin)
                }
                
                txtList[i].addTarget(self, action: #selector(umeSothukokusaiViewController.txtTapped(sender:)), for: .editingDidBegin)
                //txtTappedで処理をする
    ...
    @objc func txtTapped(sender: UITextField){
        ...
        if sender.tag < 45{
            ...
            if (txtList[sender.tag].text?.isEmpty)! {
                ...
                let DV = MainStoryboard.instantiateViewController(withIdentifier: "lecTableFromKokusaiUmeViewController") as! lecTableFromKokusaiUmeViewController
    ...
    @objc func clickInfoButton(){
        let vc = PopupKokusaiViewController(nibName: "PopupKokusaiViewController", bundle: nil)
    ...
    func textFieldSet(){
        for i in 0..<txtList.count{
            if i == 5 || i == 14 || i == 20 || i == 26 || i == 34 || i == 35 || i == 44 || i == 58 || i == 59{
    ...
    //３群のAかBの選択ボタン
    @IBAction func AorBButton(_ sender: UIButton) {
        fukiLabel = MMPopLabel(text: "3群のAかBを選択して")
        let mess = UIButton(frame: CGRect.zero)
        mess.setTitle(NSLocalizedString("14単位埋めてください", comment: "mess Button"), for: .normal)
        fukiLabel.add(mess)
        btnAorB.addSubview(fukiLabel)
    ...
    //４群と５群
    @IBAction func fourAndFiveButton(_ sender: UIButton) {
        fukiLabel = MMPopLabel(text: "4群と5群から自由に")
        let mess = UIButton(frame: CGRect.zero)
        mess.setTitle(NSLocalizedString("16単位埋めてください", comment: "mess Button"), for: .normal)
        fukiLabel.add(mess)
        btn4and5.addSubview(fukiLabel)
    ...
    func pickerView(_ pickerView: UIPickerView,
        ...
        userDefault.set(row + 6,forKey:"selectGunKokusaiNum")
        userDefault.set(gunnList[row],forKey:"selectGunKokusai")
