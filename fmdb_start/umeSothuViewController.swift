//
//  umeSothuViewController.swift
//  fmdb_start
//
//  Created by 山本シェーン on 2018/07/24.
...
class umeSothuViewController: UIViewController,UITextFieldDelegate,MMPopLabelDelegate,UIPickerViewDelegate,UIPickerViewDataSource{
    ...
    //左のラベルのリスト
    @IBOutlet var labTypeList: [UILabel]!
    
    
    //１〜５群のタイプの下に隠れているボタン
    @IBOutlet weak var btnOneFive: UIButton!
    //専攻タイプの下に隠れているボタン
    @IBOutlet weak var btnSennkou: UIButton!
    ...
    @IBOutlet weak var karakaraBottom: NSLayoutConstraint!
    ...
    let gunnList:[String] = ["2群","3群","4群","5群"]
    ...
    override func viewDidLoad() {
        ...
        umeKomaHeight = typeView.bounds.height / 20.0
        ...
        //専攻群のとこの文字の初期設定
        if (userDefault.string(forKey: "selectGunKeiei") != nil){
            typeThree.text = userDefault.string(forKey: "selectGunKeiei")! + "専攻※"
        }else{
            typeThree.text = "専攻群選択※"
        }
        ...
        let doneItem = UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(umeSothuViewController.kannryoTapped))
        ...
        for i in 0..<typeOne.count{
            typeOne[i].heightAnchor.constraint(equalTo: typeView.heightAnchor, multiplier: 1.0/20.0).isActive = true
        }
        ...
        typeThree.heightAnchor.constraint(equalTo: typeView.heightAnchor, multiplier: 3.0/20.0).isActive = true
    ...
    @objc func clickEditButton(){
        ...
        for i in 0..<txtList.count{
            ...
            if (txtList[i].text?.isEmpty)!{
                ...
                //専攻群の埋めのところで専攻群が未選択の時は押したらカラカラを出す
                if (userDefault.integer(forKey: "selectGunKeieiNum") == 0){
                    //専攻群のtextField
                    if i >= 36 && i <= 43{
                        txtList[i].addTarget(self, action: #selector(umeSothuViewController.sennkouButton), for: .editingDidBegin)
                        continue
                    }
                //群選択がされている時はカラカラ出す処理を削除しておく
                }else{
            
                    txtList[i].removeTarget(self, action: #selector(umeSothuViewController.sennkouButton), for: .editingDidBegin)
                }
    ...
    @IBAction func sennkouButton(_ sender: UIButton) {
        fukiLabel = MMPopLabel(text: "2~5群から1つ選択して")
        let mess = UIButton(frame: CGRect.zero)
        mess.setTitle(NSLocalizedString("16単位埋めてください", comment: "mess Button"), for: .normal)
        fukiLabel.add(mess)
        btnSennkou.addSubview(fukiLabel)
        ...
        //選択させるためのカラカラを出す
        karakaraTop.constant =  typeView.bounds.height - umeKomaHeight * 3.3
    ...
    func pickerView(_ pickerView: UIPickerView,
        ...
        userDefault.set(row + 5,forKey:"selectGunKeieiNum")
        userDefault.set(gunnList[row],forKey:"selectGunKeiei")
